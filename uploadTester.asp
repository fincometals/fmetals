...
function OutputForm()
%>
    <form name="frmSend" method="POST" enctype="multipart/form-data" accept-charset="utf-8" action="uploadTester.asp?RecNo=<%=strRecNo%>&Jnam=<%=strJNam%>&Pwd=<%=strPwd%>&EML=<%=strEML%>" onSubmit="return onSubmitForm();">
...
function TestEnvironment()
    ...
    if not fso.FolderExists(uploadsDirVar) then
        TestEnvironment = "<B>Folder " & uploadsDirVar & " does not exist.</B><br>The value of your uploadsDirVar is incorrect. Open uploadTester.asp in an editor and change the value of uploadsDirVar to the pathname of a directory with write permissions." & Response.write(Server.MapPath("uploadTester.asp"))
        exit function
    end if
...
<form action="login.asp" method="post" target="_self">
<input type="hidden" name="Uname" value="<%=strEML%>">
<input type="hidden" name="UPwd" value="<%=strPwd%>">
<input type="hidden" name="checker" value="yes">
<input type="hidden" name="Jview" value="<%=strJnam%>">
