...
If strRecNo = "" Then 
 ...
 End If
  ...
  uploadsDirVar = "D:\Hosting\7626218\html\files" 
...
function OutputForm()
%>
    <form name="frmSend" method="POST" enctype="multipart/form-data" accept-charset="utf-8" action="uploadTester3.asp?RecNo=<%=strRecNo%>&Jnam=<%=strJNam%>&Pwd=<%=strPwd%>&EML=<%=strEML%>" onSubmit="return onSubmitForm();">
...
function TestEnvironment()
    ...
    if not fso.FolderExists(uploadsDirVar) then
        TestEnvironment = "<B>Folder " & uploadsDirVar & " does not exist.</B><br>The value of your uploadsDirVar is incorrect. Open uploadTester3.asp in an editor and change the value of uploadsDirVar to the pathname of a directory with write permissions." & Response.write(Server.MapPath("uploadTester3.asp"))
        exit function
    end if
    ...
    If Err.Number<>0 then
        TestEnvironment = "<B>Folder " & uploadsDirVar & " does not have write permissions.</B><br>The value of your uploadsDirVar is incorrect. Open uploadTester3.asp in an editor and change the value of uploadsDirVar to the pathname of a directory with write permissions."
        exit function
    end if
    Err.Clear
    testFile.Close
...
<br><br>
<div style="border-bottom: #A91905 2px solid;font-size:16">Upload your files for job <%=strRecNo%>-<%=strJNam%>.</div>
<%
Dim diagnostics
...
<input type="hidden" name="pass" value="<%=strPwd%>">
